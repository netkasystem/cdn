(function (cc) {
  ...
  cc.Popup = (function () {
    ...
    var defaultOptions = {
      ...
      elements: {
        ...
        link_2: '<a aria-label="learn more about cookies" tabindex="0" class="cc-link" href="{{href_2}}" target="_blank">{{link_2}}</a>',
        messagelink: '<span id="cookieconsent:desc" class="cc-message">{{message}}<a aria-label="learn more about cookies" tabindex="0" class="cc-link" href="{{href}}" target="_blank">{{link}}</a><a aria-label="learn more about cookies" tabindex="0" class="cc-link" href="{{href_2}}" target="_blank">{{link_2}}</a></span>',
        dismiss: '<a aria-label="dismiss cookie message" tabindex="0" class="cc-btn cc-dismiss">{{dismiss}}</a>',
        allow: '<a aria-label="allow cookies" tabindex="0" class="cc-btn cc-allow">{{allow}}</a>',
        deny: '<a aria-label="deny cookies" tabindex="0" class="cc-btn cc-deny">{{deny}}</a>',
        close: '<span aria-label="dismiss cookie message" tabindex="0" class="cc-close">{{close}}</span>',
        setting: '<a aria-label="setting cookies" tabindex="0" class="cc-btn ccb__edit">{{setting}}</a>',
      ...
      layouts: {
        ...
        'basic-header': '{{header}}{{message}}{{link}}{{link_2}}{{compliance}}',
  ...
  function GetAllDomain() {
    ...
    if (!full_domain) return [];
    while (full_domain.includes(".")) {
      domain_all.push('.' + full_domain, full_domain);
      full_domain = full_domain.slice(full_domain.indexOf(".") + 1);
    }
    return domain_all;
  }
  ...
  function RemoveCookies(list_allow_cookie) {
    ...
    var dm_all = GetAllDomain();
    for (var i = 0; i < cookies.length; i++) {
      var cookie = cookies[i];
      var name = cookie.slice(0, cookie.indexOf("=")).trim();
      if (!list_allow_cookie.includes(name)) {
        // document.cookie = name + "=;expires=Thu, 01 Jan 1970 00:00:00 GMT";
        dm_all.map(function(dm){
          document.cookie = name + '=;expires=Thu, 01 Jan 1970 00:00:01 GMT;domain=' + dm;
        });
      }
    }
  }
